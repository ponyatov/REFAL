...
*App=F18(K u 14 K O a eE lB oA s k *App dD gG lB p P p)
*I=F5(O SCB O gG p)
*K=F4(K O dD p)
...
*GCD=F44(G u 6 C 0 P dD p u 32 G u 16 P r k *GCDSTEP eE lA oA G G O k *GCD eK lB p O k *GCD k *GCD eE lA eG lA oB O gM p P dD p)
*ORT=F100(G u 8 C 0 P dE i 1 p G P k *GCDSTEP eE lA oA G G G G G G P w 45 u 8 SJL C 0 P dM dO x k *ORT eK lA oB G G P k *ADD k *MUL dT dL lB k *MUL dU dM lB lB k *ADD k *MUL dT dN lB k *MUL dU dO lB lB x oC G G P k *SIGN dT lA oD u 6 C 1 P eU p C -1 P k *MUL dZ dT lB k *MUL dZ dU lB p)
*REM=F8(G G P k *MOD gE lB p)
*DIVREM=F8(G G P k *DIVMOD gE lB p)
...
*NAME=F30(G P s u 15 k *TABLE_NAME jB *SYSID 'O' dD lA oA G P SHG O gK p k *IMPLODE k *TO_CHARS dD lA lA oB O gJ p)
*RENAME=F21(G G P k *POS i *SYSTABLE dD lB oA s G P k *SETN i *SYSTABLE dJ dE lB p)
<*RENAME APPLY EXECUTE>
<*RENAME ENCODE ENCODE0>
*APPLY=F8(O a eE lA oA O gK p)
*ENCODE=F28(G O s u 14 k *MODE dD lA oA C TABLE P k *ENCODE0 hDF lB p k *ENCODE i *SYSID eF lA oB O gL p)
*TO_WORD=F11(O k *SETV i X k *TO_CHARS gE lB lB p)
...
<BlockBegin Map I OPEN CLOSE APPEND LOADNAME Save UnSave EndOfSession Input INCLUDE FileExt LOAD MODTABLE ButFirst DELETE CTIME B RFStart CONTINUE ASK LINK CALL OUT LINKTABLE GETENV TO_CHARS WRITE! READ! STDLOAD EOF SYSTABLE READLN! GETV MOVE SETV SET NEW MODE TYPE LOWER WRITELN TABLE_VALUE STDERR PRINTLN! TIME_ELAPSED SUB PRINT EXECUTE BREAK SYSARG GETN EXIT PRINTLN UPPER>
*LOADNAME=S0()
...
*NormParam=F41(G P k *MODE dD lA oA C STRING P k *TYPE k *GETV dD lA lA oB u 13 C 'O' G O dO k *SETV i X eQ lB p G O k *SETV i X eP lB p)
...
*MODTABLE=T0()
*LoadFile=F86(G P k *NormParam dD lA oA u 10 C '*' G P k *ActPrint dJ lB p u 10 C '@' G P k *ActPrint dJ lB p u 18 C '!' G P k *LoadMod i *WRITELN k *NormName dJ lA lA oB O gP p u 18 C '^' G P k *LoadMod i *WRITELN k *NormName dJ lA lA oC O gP p u 16 G P k *LoadMod i *I k *NormName dI lA lA oD O gO p O p)
...
*CTIME=F45(K O k *LINK dD lA oA K P k *TIME_ELAPSED lA a dK eF lA k *TIME_ELAPSED lA oB G H O k *PRINT i "Computation time = " k *Tosec k *TO_CHARS k *SUB dQ dP lB lB lB i " seconds.
" lB oC s eS p)
*B=F13(K O k *EXECUTE i *PreBreak k *LINK dD lB gF lB p)
...
*LINK=F117(K P k *MODE dD lA oA u 105 C STRING P k *TABLE_VALUE jB *SYSTABLE 'U' dD lA oB G P k *MODE dN lA oC u 81 C EMPTY P u 41 k *TABLE_VALUE jB *LINKTABLE 'O' dD lA oD G P k *LOAD dX lA oE u 11 k *MODE dN lA oF C FUNC P dN p k *PRINTLN i "ERROR: No function " dD i " in module " dX lA oG i *OUT p k *SETV i X k *UPPER k *GETV dD lA lA lA oH u 14 MD P k *PRINTLN i "FAIL: Unknown function name " dD lB oI s i *OUT p K P k *LINK dX lB p G P dN p G P dD p)
*CALL=F13(K O a k *LINK dD lA eF lA oA O gL p)
*OUT=F2(O p)
<BlockEnd>
*LINKTABLE=T0()
<*TABLE_LINK *LINKTABLE COMP C CL C BRK TR TRC TR TRCA TR>
*TRACE=F8(O k *App i *TRC gE lB p)
<*RFStart>
